...
fit_fims_scenario <- function(om_setup, truth_list, sim_list, index_sd, catch_ss, survey_ss, fish_sel_vec, srv_sel_vec, init_naa_vec, log_sd_proxy) {
  ...
  landings_data <- data.frame(
    type = "landings", fleet = fishing_fleet_name, age = NA, timing = model_years,
    value = sim_list[["catch_index"]][["sampled_value"]], unit = "mt", uncertainty = index_sd
  )
  
  index_data <- data.frame(
    type = "index", fleet = survey_fleet_name, age = NA, timing = model_years,
    value = sim_list[["survey_index"]][["sampled_value"]], unit = "mt", uncertainty = 0.1
  )
  
  age_data <- rbind(
    data.frame(
      type = "age_comp", fleet = fishing_fleet_name, age = unname(ages[sim_list[["catch_agecomp"]][["truth_group"]]]),
      timing = sim_list[["catch_agecomp"]][["truth_year"]], value = sim_list[["catch_agecomp"]][["sampled_value"]],
      unit = "number", uncertainty = catch_ss
    ),
    data.frame(
      type = "age_comp", fleet = survey_fleet_name, age = unname(ages[sim_list[["survey_agecomp"]][["truth_group"]]]),
  ...
  weight_at_age <- data.frame(
    type = "weight_at_age", fleet = fishing_fleet_name, age = unname(ages[truth_list[["weight_agecomp_om"]][["truth_group"]]]),
  ...
  updated_parameters <- default_parameters |>
    ...
    dplyr::rows_update(
      y = tibble::tibble(
        fleet = fishing_fleet_name, label = "log_Fmort",
